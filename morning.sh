...
mkdir -p /boot/grub
INITRD_LINE=""
INITRD_FILE=$(ls /boot/initr{d,amfs}-* 2>/dev/null | head -1 | xargs basename 2>/dev/null || true)
if [[ -n "${INITRD_FILE}" ]]; then
    INITRD_LINE="    initrd /boot/${INITRD_FILE}"
    echo "[CHROOT] initrd 検出: ${INITRD_FILE}"
else
    echo "[CHROOT] initrd なし（モジュールが全て組み込みである必要があります）"
fi
